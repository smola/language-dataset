            ...
            if (arg.at(1)=="-") then {
                match (arg)
                    case { "-i" ->
                        ...
                        print "  -i <path>      The Grace file to process (with path relative to ./minigrace executable)"
                        ...
                        print "  [--publiconly] Optional. If set, only public methods are documented and public variables are listed as methods. Default is off."
...
class directoryBuildingVisitor.createFrom(in)outTo(dir)as(pageType) {
    inherits ast.baseVisitor

    var isClass := false
    var isType := false
    if (pageType=="class") then { isClass := true }
    elseif (pageType=="type") then { isType := true }

    def pageName = if (in.endsWith(".grace").not) then { in }
                   else { in.substringFrom(0)to(in.size - 6) }
    def title = if (isType) then { "Type: {pageName}" }
                elseif (isClass) then { "Class: {pageName}" }
                else { "Module: {pageName}" }

    def outdir = if (isClass || isType) then { dir } else { pageName }
    ...
    method visitTypeDec(o) -> Boolean {
        if (isType == false) then {
            def typeVis = directoryBuildingVisitor.createFrom("{o.name.value}")outTo("{outdir}")as("type")
            o.accept(typeVis)
            typeVis.generate
            return false
        } else { return true }
    ...
    method visitClass(o) -> Boolean {
      if (isClass == false) then {
        ...
        def classVis = directoryBuildingVisitor.createFrom("{o.name.value}")outTo("{outdir}")as("class")
        o.accept(classVis)
        classVis.generate
        return false
      } else { return true }
...
class graceDocVisitor.createFrom(in)outTo(dir)as(pageType) {
    ...
    //debugging
    if (settings.verbosity > 1) then { print "On {title} - graceDocVisitor created... ({sys.cputime})" }
    ...
    method buildTemplate is confidential {
        ...
        out := out ++ "<meta charset=\"UTF-8\" />"
        var cssDir
        if (isClass || isType) then { cssDir := "../../gracedoc.css" }
...
.quiet {
    ...
    method generate is public {
        if (settings.verbosity > 1) then { print "On {title} - starting to assemble HTML ({sys.cputime})" }
        ...
        outfile.close
        if (settings.verbosity > 1) then { print "On {title} - file written ({sys.cputime})" }
    }
        ...
        if (isType == true) then {
                    ...
                    for (part.params) do { param ->
                        if (param.dtype != false) then {
                            ...
                            if (param.dtype.kind == "identifier") then {
                                ...
                                param.dtype.params.do { each -> t := "{t}{getTypeLink(each.value)}" } separatedBy { t := t ++ ", " }
                                t := t ++ "&gt;"
                            }
                        } else {
                            t := t ++ "<span class='parameter-name'>" ++ param.nameString ++ "</span>"
            ...
            if (o.rtype != false) then {
                if (o.rtype.kind == "identifier") then {
                    ...
                    o.rtype.params.do { each -> t := "{t}{getTypeLink(each.value)}" } separatedBy { t := t ++ ", " }
                    t := t ++ "&gt;"
                }
            } else {
                t := t ++ "Done"
            ...
            t := t ++ "</code></td></tr>"
            if (o.comments.size > 0) then {
                t := t ++ "<tr class='description'><td colspan='2'>"
                for (o.comments) do { c ->
                    t := t ++ trim(c.value) ++ " "
                }
                t := t ++ "</td></tr>"
    ...
    method visitTypeDec(o) -> Boolean {
        if (isType == false) then {
            ...
            if (false != o.generics) then {
                t := t ++ "&lt;"
                for (o.generics.params) do { g ->
                    t := t ++ g.nameString
                    if (g != o.generics.params.last) then { t := t ++ ", " }
                }
                t := t ++ "&gt;"
            }
            t := t ++ "</td></tr>"
            ...
            if (o.comments.size > 0) then {
                t := t ++ "<div class='top-box-description'>"
                for (o.comments) do { c ->
                    t := t ++ trim(c.value) ++ " "
                }
                t := t ++ "</div>"
    ...
    method visitClass(o) -> Boolean {
      if (isClass == false) then {
        var t := "<tr class='placeholder'>"
        def n = o.name.value
        t := t ++ "<td><code><span class='class-name'>{getClassLink(o.name.value)}</span>"
        t := t ++ "."
        for(o.signature) do { part ->
            t := t ++ "<span class='method-name'>{part.name}</span>"
            if (part.params.size > 0) then {
        ...
        if (o.dtype != false) then {
            ...
            if (o.dtype.kind == "identifier") then {
                ...
                o.dtype.params.do { each -> t := "{t}{getTypeLink(each.value)}" } separatedBy { t := t ++ ", " }
                t := t ++ "&gt;"
            }
        }
        t := t ++ "</code></td></tr>"
        ...
        def classVis = graceDocVisitor.createFrom("{o.name.value}")outTo("{outdir}")as("class")
        o.accept(classVis)
        classVis.generate
    ...
    method visitDefDec(o) -> Boolean {
        if (isClass == true) then {
            if (!settings.publicOnly) then {
                ...
                if (o.comments.size > 0) then {
                    t := t ++ "<tr class='description'><td colspan='3'>"
                    for (o.comments) do { c ->
                        t := t ++ trim(c.value) ++ " "
                    }
                    t := t ++ "</td></tr>"
            ...
            if (!settings.publicOnly) then {
                ...
                if (o.dtype != false) then {
                    if (o.dtype.kind == "identifier") then {
                        ...
                        t := t ++ getTypeLink(o.dtype.value.value) ++ "&lt;"
                        o.dtype.params.do { each -> t := "{t}{each.value}" } separatedBy { t := t ++ ", " }
                        t := t ++ "&gt;"
                    }
                }
                t := t ++ "</code></td></tr>"
...
//var allmodules := list.with(settings.inputdir)
var allfilesparsed := dictionary.empty
...
for (allmodules) do { filename ->
    if (filename.endsWith(".grace")) then {
        ...
        //file := io.open("{settings.inputdir}", "r")
        //if (settings.verbosity > 0) then { print "On {filename} - lexing... ({sys.cputime})" }
        var tokens := lexer.new.lexfile(file)
        //if (settings.verbosity > 0) then { print "On {filename} - done lexing... ({sys.cputime})" }
        if (settings.verbosity > 0) then { print "On {filename} - parsing... ({sys.cputime})" }
        //allfilesparsed.at(counter)put(list.empty)
        //var values := parser.parse(tokens)
        allfilesparsed.at(counter)put(parser.parse(tokens))
        
        if (settings.verbosity > 0) then { print "On {filename} - done parsing... ({sys.cputime})" }
        counter := counter + 1
...
for (allmodules) do { filename ->
    if (filename.endsWith(".grace")) then {
        if (settings.verbosity > 0) then { print "On {filename} - building directories... ({sys.cputime})" }
        modulename := filename.substringFrom(1)to(filename.size - 6)
        var values := allfilesparsed.at(counter)
        dbv := directoryBuildingVisitor.createFrom(filename)outTo(modulename)as("module")
        for (values) do { v ->
            v.accept(dbv)
            //print(v.pretty(0))
        }
        dbv.generate
        if (settings.verbosity > 0) then { print "On {filename} - directories built... ({sys.cputime})" }
        counter := counter + 1
    }
}
...
for (allmodules) do { filename ->
    if (filename.endsWith(".grace")) then {
        if (settings.verbosity > 0) then { print "On {filename} - generating GraceDocs... ({sys.cputime})" }
        modulename := filename.substringFrom(1)to(filename.size - 6)
        var values := allfilesparsed.at(counter)
        ...
        gdv.buildjs
        gdv.build404
        if (settings.verbosity > 0) then { print "On {filename} - done! ({sys.cputime})" }
        counter := counter + 1
    }
}
